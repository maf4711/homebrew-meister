#!/usr/bin/env bash
# Build MeisterAI.app (OnyX-style GUI) and optionally install to /Applications
set -euo pipefail
...
APP=$(find build/DerivedData/Build/Products/Release -name "MeisterAI.app" -maxdepth 2 | head -1)
if [ -z "$APP" ] || [ ! -d "$APP" ]; then
  echo "ERROR: MeisterAI.app not found"
  exit 1
fi
...
OUT="$ROOT/dist"
mkdir -p "$OUT"
rm -rf "$OUT/MeisterAI.app"
cp -R "$APP" "$OUT/MeisterAI.app"

# Ad-hoc sign for local Gatekeeper friendliness
codesign --force --deep --sign - "$OUT/MeisterAI.app" 2>/dev/null || true

echo "==> dist: $OUT/MeisterAI.app"
...
if $cask_release; then
  ...
  codesign --force --deep --options runtime --timestamp \
    --sign "$SIGN_ID" "$OUT/MeisterAI.app"
  codesign --verify --deep --strict "$OUT/MeisterAI.app"
  ZIP="$OUT/MeisterAI-macOS.zip"
  rm -f "$ZIP"
  ditto -c -k --keepParent "$OUT/MeisterAI.app" "$ZIP"
  ...
  xcrun notarytool submit "$ZIP" \
    ...
    --wait
  echo "==> staple"
  xcrun stapler staple "$OUT/MeisterAI.app"
  rm -f "$ZIP"
  ditto -c -k --keepParent "$OUT/MeisterAI.app" "$ZIP"
  echo "==> zip: $ZIP"
  shasum -a 256 "$ZIP"
...
if $do_install; then
  ...
  rm -rf /Applications/MeisterAI.app
  cp -R "$OUT/MeisterAI.app" /Applications/
  xattr -dr com.apple.quarantine /Applications/MeisterAI.app 2>/dev/null || true
  echo "    open -a MeisterAI"
fi
