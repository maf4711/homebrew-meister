...
Native **macOS GUI** (SwiftUI) im **OnyX-Stil** über die CLI `MeisterAI`.
...
Siri-Formulierungen enthalten den App-Namen MeisterAI. Ohne Bericht wird das
ausdrücklich zurückgemeldet; ein alter Bericht wird nicht als Live-Check ausgegeben.

## Voraussetzung

```bash
brew install maf4711/meister/meister   # liefert MeisterAI CLI
```

Die App sucht (in dieser Reihenfolge):

1. `/opt/homebrew/bin/MeisterAI`
2. `/usr/local/bin/MeisterAI`
3. `~/Developer/homebrew-meister/MeisterAI.sh`
4. Fallback `meister`
...
Nur bauen nach `dist/MeisterAI.app`:
...
- **Tweaks:** `MeisterAI tweaks <name> on|off` (identisch zur CLI)
...
```bash
# falls blockiert:
xattr -dr com.apple.quarantine /Applications/MeisterAI.app
