...
#  - LaunchAgent -I: daily meister --auto -q + weekly --deep -q;
...
#  - meister bloatware | kill-bloat | bloat  (lib/commands/bloatware.sh)
...
#  - Opt-out: TOUCHID_SUDO=false in ~/.meister/config or meister touchid --off
#
# NEW in v6.16 — twin benchmark + heald handshake fields:
#  - meister twins-bench | bench-twins  (scripts/twin-benchmark.sh)
...
#  - selftest twin-safe (meister + meister); explain uses AI_BACKEND_LABEL
...
#  - meister why | storage | contacts doctor | report --diff|--json | doctor --json
...
# Must resolve brew symlinks (/opt/homebrew/bin/meister → Cellar/...).
...
if [ "${1:-}" = "bloatware" ] || [ "${1:-}" = "kill-bloat" ] || [ "${1:-}" = "bloat" ]; then
    echo -e "\033[1;34m  meister BLOATWARE — catalog scan / quarantine kill\033[0m"
    echo ""
...
module_tm_health() {
    ...
    if tmutil destinationinfo 2>&1 | grep -qi "No destinations"; then
        ...
        report_add WARN "Time Machine: not configured (no backup!) → meister backup"
    ...
    if [ -z "$latest" ]; then
        ...
        report_add WARN "Time Machine: no backups → meister backup"
    else
...
# long timeout + !tty_tickets so meister and meister share credentials.
SUDO_AUTHED=false
...
ensure_touchid_sudo() {
    ...
    log WARN "Could not enable Touch ID for sudo (manual: meister touchid)"
    return 1
}
...
print_report() {
    ...
    echo "Abschlussbericht mit Belegen: meister ai report"
}
...
PLISTEOF
    ...
    if [ "$ok" -ge 1 ]; then
        ...
        echo "  Daily:  09:15  ${daily_args}   (meister auto)"
...
if [ "${1:-}" = "doctor" ]; then
    ...
    if touchid_sudo_enabled 2>/dev/null || { [ -f /etc/pam.d/sudo_local ] && grep -qE '^[[:space:]]*auth[[:space:]].*pam_tid' /etc/pam.d/sudo_local 2>/dev/null; }; then
        ...
        _check warn "Touch ID sudo" "off — auto-enable on next sudo (or: meister touchid)"
    else
        _check warn "Touch ID sudo" "off (TOUCHID_SUDO=false) — meister touchid"
    fi
...
if [ "${1:-}" = "selftest" ]; then
    ...
    _SELF="$0"
    _BIN=$(basename "$_SELF")
    # sync-twins rewrites "meister"→"meister"; build OTHER via concat so it survives.
...
if [ "${1:-}" = "ai-log" ] || [ "${1:-}" = "ai-usage" ] || { [ "${1:-}" = "ai" ] && { [ "${2:-}" = "usage" ] || [ "${2:-}" = "log" ]; }; }; then
    ...
    echo "  meister ai report  → gespeicherte Ergebnisse mit Belegverweisen"
...
if [ "${1:-}" = "ai" ]; then
    ...
    if $_AI_DIAG_ONLY; then
        echo -e "\033[1;34m  meister AI — Nur Diagnose\033[0m"
    else
...
if [ "${1:-}" = "touchid" ]; then
    ...
    if touchid_sudo_enabled 2>/dev/null || grep -qE '^[[:space:]]*auth[[:space:]].*pam_tid' "$_PAM_LOCAL" 2>/dev/null; then
        ...
        echo "  Default: every meister/meister run keeps this on (TOUCHID_SUDO=true)"
...
if [ "${1:-}" = "sudo-setup" ]; then
    echo -e "\033[1;34m  meister SUDO-SETUP — shared sudo ticket\033[0m"
    echo ""
    _SUDOERS="/etc/sudoers.d/zz-meister"
    _user=$(id -un)
    _body=$(printf '%s\n' \
        "# meister / meister — share sudo across TTYs for ${_user}" \
    ...
    if sudo -n true 2>/dev/null; then
        ...
        echo "  Run once in Terminal: meister sudo-setup"
        rm -f "$_tmp"
        exit 0
    fi
    rm -f "$_tmp"
...
# ── twins-bench — meister vs meister (Ollama vs Apple Intelligence) ──
...
if [ "${1:-}" = "why" ]; then
    echo -e "\033[1;34m  meister WHY\033[0m"
    echo ""
...
if [ "${1:-}" = "storage" ]; then
    echo -e "\033[1;34m  meister STORAGE — safe-to-delete candidates\033[0m"
    echo ""
...
if [ "${1:-}" = "contacts" ]; then
    echo -e "\033[1;34m  meister CONTACTS\033[0m"
    echo ""
    case "${2:-doctor}" in
        doctor|health|status|"")
            ...
            ;;
        *)
            echo "Usage: meister contacts doctor"
            exit 1
            ;;
    esac
    exit 0
...
if [ "${1:-}" = "report" ]; then
    ...
    if [ "$_N" = "--diff" ] || [ "${2:-}" = "--diff" ]; then
        echo -e "\033[1;34m  meister REPORT --diff\033[0m"
        echo ""
    ...
    case "$_N" in *[!0-9]*) echo "Usage: meister report [N|--diff|--json [N]]"; exit 1 ;; esac
    _hist="$MEISTER_DIR/history.log"
...
MAINTENANCE:
  ...
  meister --ai-heal-execute  Allow AI-Heal / Learned-Fix to RUN commands
                                 (default: suggest-only; safer)
  meister --ai-heal-suggest  Force suggest-only (overrides config)
...
TOOLS:
  ...
  meister simfix       Stop stale Simulator.app (no simctl catalog, does not boot)
  ...
  meister report --diff  Delta between last two runs (OK/FIX/WARN/ERR/SCORE)
  meister report --json [N]  History as JSON array
  meister why <x>      Why a module runs/skips, or explain a warn (profile|Homebrew|…)
  meister storage      Safe-to-delete size candidates (DerivedData, caches, …)
  meister contacts doctor  AddressBook size / bloat risk (read-only)
  ...
  meister doctor       Read-only system checklist (security, brew, TM, AI)
  meister doctor --json  Same signals as machine-readable JSON
  meister twins-bench [--json] [--quick]  Benchmark meister vs meister
...
APP MANAGEMENT:
  ...
  meister bloatware [scan|kill] [--p0|--p1] [--dry-run] [-y] [--json|--tsv]
