...
#  - MeisterAI homebrew-share runs the same heal. No sudo ticket (LaunchAgent)
...
# NEW in v6.23 — macOS 27 Foundation Models for MeisterAI:
...
# NEW in v6.20 — close daily-runner loops (MeisterAI is the product):
...
#  - LaunchAgent -I: daily MeisterAI --auto -q + weekly --deep -q;
...
#  - MeisterAI bloatware | kill-bloat | bloat  (lib/commands/bloatware.sh)
...
#  - Opt-out: TOUCHID_SUDO=false in ~/.meister/config or MeisterAI touchid --off
#
# NEW in v6.16 — twin benchmark + heald handshake fields:
#  - MeisterAI twins-bench | bench-twins  (scripts/twin-benchmark.sh)
...
#  - selftest twin-safe (meister + MeisterAI); explain uses AI_BACKEND_LABEL
...
#  - MeisterAI why | storage | contacts doctor | report --diff|--json | doctor --json
...
#  - MeisterAI ai usage | ai-log  — human summary of AI activity
...
#  - AI-Heal stays on in quick/auto/deep (backend: Apple on MeisterAI, Ollama on meister)
#  - LaunchAgents: daily --quick + weekly --deep (meister keep-current)
#  - twins: edit MeisterAI.sh → scripts/sync-twins.sh (injects Ollama into meister.sh)
#
...
#  - keep_sudo every 30s (was 60); reuses existing ticket across meister + MeisterAI
#  - free/heal/remove/orphans/simfix share ensure_sudo (no re-prompt if ticket live)
#  - MeisterAI sudo-setup: install /etc/sudoers.d/zz-meister
#      timestamp_timeout=120 + !tty_tickets (share auth across terminals)
#
# Date: 2026-07-27
#
# Twin of meister.sh: same maintenance modules, branded as MeisterAI.
...
#  - Full MeisterAI branding on all tool banners
#  - MeisterAI today   — morning briefing (score, AI, disk, updates, top warns)
#  - MeisterAI doctor  — read-only system checklist (security, brew, TM, AI, …)
#  - MeisterAI selftest — smoke-test the CLI itself
#  - MeisterAI suggest <text> — AI fix suggestion (read-only, no exec)
#  - MeisterAI privacy — privacy grants / FDA quick audit
#
#
# NEW in v6.0 — "MeisterAI": Ollama fully replaced by Apple Intelligence
...
#  - MeisterAI explain / MeisterAI ai / AI-Heal all run on-device via a tiny Swift
...
# Must resolve brew symlinks (/opt/homebrew/bin/MeisterAI → Cellar/...).
...
if _meister_lib_ok "${MEISTER_LIB_DIR:-}"; then
    ...
    # shellcheck source=/dev/null
    [ -f "$MEISTER_LIB_DIR/core/fm_diagnosis.sh" ] && . "$MEISTER_LIB_DIR/core/fm_diagnosis.sh"
    # shellcheck source=/dev/null
...
if [ "${1:-}" = "bloatware" ] || [ "${1:-}" = "kill-bloat" ] || [ "${1:-}" = "bloat" ]; then
    echo -e "\033[1;34m  MeisterAI BLOATWARE — catalog scan / quarantine kill\033[0m"
    echo ""
...
# Apple Intelligence (FoundationModels) — on-device LLM, replaces Ollama (MeisterAI)
FM_ENABLED=true
...
ai_usage_record() {
    ...
    detail="run_id=${RUN_ID:-none} $(printf '%s' "$detail" | tr '\n' ' ' | cut -c1-200)"
...
# ===== TWIN:AI-BACKEND (Apple Intelligence — MeisterAI) =====
...
_fm_helper_source() {
    cat "$MEISTER_LIB_DIR/fm/MeisterFM.swift"
}
...
ensure_fm_helper() {
    ...
    mkdir -p "$MEISTER_DIR"
    local want; want=$(_fm_helper_source) || { FM_ENABLED=false; return 1; }
...
fm_available() {
    ...
    "$FM_HELPER" --check --model "${1:-system}" 2>/dev/null
}
...
fm_query() {
    ensure_fm_helper || { ai_usage_record query readonly unavailable "helper build unavailable"; return 69; }
    local prompt="$1" label="${2:-query}" mode="${3:-readonly}" purpose
    purpose="$label"
    case "$label" in
        AI-Heal:*|ai-heal*|heal:*) purpose=ai-heal; mode=heal-candidate ;;
        explain*) purpose=explain ;; ai-diagnose*|diagnose*) purpose=ai-diagnose ;;
        today*) purpose=today ;; suggest*) purpose=suggest ;;
    esac
    local model=system budget=35 resp err rc=0 kind start elapsed
    case "$purpose" in
        ai-heal) model="${FM_HEAL_MODEL:-auto}" ;;
        ai-diagnose) model="${FM_DIAGNOSE_MODEL:-system}" ;;
    esac
    case "$model" in system) ;; auto|pcc) budget=80 ;; *) return 64 ;; esac
    ...
    start=$(date +%s)
    ai_usage_record "$purpose" "$mode" request "model=$model budget_sec=$budget"
    ai_trace_line "FM: purpose=$purpose route=$model budget=${budget}s"
    err=$(mktemp "${TMPDIR:-/tmp}/meister-fm.XXXXXX") || return 1
    if ! command -v timeout >/dev/null 2>&1; then
        rm -f "$err"
        ai_usage_record "$purpose" "$mode" unavailable "bounded process runner missing"
        return 69
    fi
    resp=$(printf '%s' "$prompt" | timeout "$budget" "$FM_HELPER" --purpose "$purpose" --model "$model" 2>"$err") || rc=$?
    elapsed=$(( $(date +%s) - start ))
    while IFS= read -r line; do
        case "$line" in fm-meta\ *) ai_usage_record "$purpose" "$mode" meta "$line" ;; esac
    done < "$err"
    if [ "$rc" -ne 0 ]; then
        kind=$(sed -n 's/^fm-error kind=\([a-z-]*\).*/\1/p' "$err" | head -1)
        case "$rc" in 124|137) kind=timeout ;; esac
        case "$kind" in timeout|unavailable|context|guardrail|generation|invalid-input) ;; *) kind=generation ;; esac
        ai_usage_record "$purpose" "$mode" "$kind" "exit=$rc duration_sec=$elapsed route=$model"
        ai_trace_line "FM: $kind (exit=$rc, ${elapsed}s); keine verwertbare Antwort"
        rm -f "$err"
        return "$rc"
    fi
    rm -f "$err"
    if [ -z "$resp" ]; then
        ai_usage_record "$purpose" "$mode" response-empty "duration_sec=$elapsed"
        return 65
    fi
    ai_usage_record "$purpose" "$mode" response-ok "chars=${#resp} duration_sec=$elapsed route=$model"
    printf '%s' "$resp"
}
...
ai_heal() {
    ...
    if ! fm_available "${FM_HEAL_MODEL:-auto}"; then
    ...
    ai_heal_emit "###  Fehlerbelege werden vor der Modellabfrage bereinigt."
    ...
    local ai_response
    if ! command -v fm_diagnose >/dev/null 2>&1 || ! fm_diagnose "$module_name" "$error_output" "$prev_attempt" ai-heal; then
        log_heal_event "ai-heal" "$module_name" "diagnosis-failed" "No validated diagnosis available"
        return 1
    fi
    ai_response="$FM_DIAGNOSIS_COMMAND"
    ...
    if ! heal_catalog_command_allowed "$ai_response" || ! heal_command_allowed "$ai_response"; then
...
firewall_state() {
    local output
    if ! output=$(timeout 10 /usr/libexec/ApplicationFirewall/socketfilterfw --getglobalstate 2>/dev/null); then
        echo unknown
...
# 0 = activity found, 1 = no events, 2 = query unavailable. An event does not
# establish that a complete malware scan succeeded.
xprotect_recent_activity() {
    local output
    output=$(timeout 15 /usr/bin/log show --predicate 'subsystem == "com.apple.XProtectFramework"' --last 24h --style compact 2>/dev/null) || return 2
    printf '%s\n' "$output" | grep -qE '^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9]{2}:'
}
...
module_xprotect() {
    ...
    if echo "$gk_status" | grep -q "enabled"; then
        ...
        report_add WARN "Gatekeeper status requires verification"
        issues=$((issues + 1))
        if ! $DRY_RUN; then
    ...
    if [ -d "$xp_bundle" ]; then
        ...
        if [ "$xp_age_days" -gt 14 ]; then
            ...
            report_add WARN "XProtect signatures: ${xp_age_days} days old; update not yet verified"
            issues=$((issues + 1))
    ...
    if [ -d "$xpr_dir" ]; then
        ...
        local xpr_rc=0
        xprotect_recent_activity || xpr_rc=$?
        case "$xpr_rc" in
            0) log STEP "   XProtect Remediator: activity in last 24h (not proof of a completed scan)" ;;
            1) log STEP "   XProtect Remediator: no activity found in last 24h" ;;
            *) log WARN "   XProtect Remediator: activity query unavailable"
               report_add WARN "XProtect Remediator activity could not be checked"
               issues=$((issues + 1)) ;;
        esac
    else
    ...
    # 6. Firewall: reading state needs no sudo; failure means unknown.
    local fw_status
    fw_status=$(firewall_state)
    case "$fw_status" in
        enabled) log STEP "   Firewall: active" ;;
        disabled)
            log WARN "   Firewall: disabled"
            if ! $DRY_RUN && sudo -n /usr/libexec/ApplicationFirewall/socketfilterfw --setglobalstate on 2>/dev/null &&
                [ "$(firewall_state)" = enabled ]; then
                log FIX "   Firewall enabled (verified)"
                report_add FIX "macOS Firewall enabled (verified)"
            else
                report_add WARN "macOS Firewall disabled"
                issues=$((issues + 1))
            fi ;;
        *) log WARN "   Firewall: state unavailable"
           report_add WARN "macOS Firewall state could not be checked"
           issues=$((issues + 1)) ;;
    esac
...
module_tm_health() {
    ...
    if [ -z "$latest" ]; then
        ...
        report_add WARN "Time Machine: no backups → MeisterAI backup"
    else
        ...
        if [ -n "$bin" ] && [ ! -e "$bin" ]; then
            ...
            if [ "${AUTOFIX_ORPHAN_LAUNCHD:-true}" = "true" ]; then
                ...
                if ensure_sudo "orphan LaunchDaemon $label" 2>/dev/null || sudo_has_ticket 2>/dev/null; then
                    ...
                    log WARN "   Need sudo to remove $label — run: MeisterAI autofix"
                fi
            fi
        fi
    done
...
module_system_maintenance() {
    ...
    if ! sudo_has_ticket; then
        log WARN "System maintenance skipped: no sudo ticket"
        report_add WARN "System maintenance skipped: administrator authentication required"
        return 0
    fi
    local task failed=0
...
module_sleep_blockers() {
    ...
    local n=0 names="" seen_names="|" pid name
    while IFS= read -r line; do
        [ -z "$line" ] && continue
        ...
        case "$seen_names" in
            *"|$name|"*) ;;
            *) seen_names="${seen_names}${name}|"; names="${names:+$names, }$name"; n=$((n + 1)) ;;
        esac
    done <<< "$assertions"
    if [ "$n" -eq 0 ]; then
...
# long timeout + !tty_tickets so meister and MeisterAI share credentials.
SUDO_AUTHED=false
...
# ticket already exists (earlier meister / MeisterAI / manual sudo), reuse it.
...
print_report() {
    ...
    echo ""
    echo -e "${BLUE}====================================================${NC}"
    echo -e "${BLUE}   MeisterAI REPORT (v6.2)${NC}"
    ...
    if [ "${AI_CALLS_THIS_RUN:-0}" -gt 0 ] || [ -f "${AI_USAGE_LOG:-$MEISTER_DIR/ai_usage.log}" ]; then
        ...
        echo "  Anzeigen:    MeisterAI ai usage"
    ...
    echo "Nachweise: ${VERIFIED_REPAIR_COUNT:-0} verifizierte Reparaturen; ${#REPORT_AI_DIAGNOSES[@]} FM-Diagnosen."
    echo "Abschlussbericht mit Belegen: MeisterAI ai report"
}
...
health_dashboard() {
    ...
    if [ -f "${AI_USAGE_LOG:-$MEISTER_DIR/ai_usage.log}" ]; then
        ...
        echo -e "  AI-Log:  ${_ai_n} calls  (MeisterAI ai usage)"
...
log_analysis() {
    ...
    # Count distinct runs, not repeated lines. File mtimes cannot establish the
    # age of their contents; restrict timestamps as well as the last five runs.
    local cutoff recent_warns
    cutoff=$(date -v-30d +%F 2>/dev/null) || cutoff=$(date -d '30 days ago' +%F)
    local log_files=()
    [ -f "${LOGFILE}.old" ] && log_files+=("${LOGFILE}.old")
    [ -f "$LOGFILE" ] && log_files+=("$LOGFILE")
    recent_warns=""
    if [ "${#log_files[@]}" -gt 0 ]; then
        recent_warns=$(awk -v cutoff="$cutoff" '
            substr($0, 1, 10) < cutoff {next}
            / - INFO - [Mm]eister(AI|Siri)? v[^ ]+ started / {run++; next}
            run && /^[0-9-]+ [0-9:]+ - (WARN|ERROR) - / {
                message=$0
                sub(/^[0-9-]+ [0-9:]+ - (WARN|ERROR) - */, "", message)
                seen[run SUBSEP message]=1
            }
            END {
                for (key in seen) {
                    split(key, parts, SUBSEP)
                    if (parts[1] > run-5) counts[parts[2]]++
                }
                for (message in counts)
                    if (counts[message] >= 3) print counts[message] " " message
            }
        ' "${log_files[@]}" | sort -rn | head -10)
    fi
    if [ -n "$recent_warns" ]; then
        log INFO "   Recurring problems (at least 3 of last 5 runs, within 30 days):"
        while IFS= read -r line; do
            log STEP "     - ${line#* }"
        done <<< "$recent_warns"
    else
        log STEP "   No recurring problems in recent runs"
    fi
...
build_report_summary() {
    ...
    echo "MeisterAI v${MEISTER_VERSION} | ${total_mins}min | $summary"
}
...
send_report_notification() {
    ...
    send_notification "MeisterAI" "$summary" "$subtitle"
}
...
install_launchagent() {
    ...
    local script_path
    script_path=$(command -v MeisterAI 2>/dev/null || command -v meister 2>/dev/null || true)
    ...
    case "$self_base" in
        MeisterAI|MeisterAI.sh|meisterSiri|meisterSiri.sh)
            script_path=$(command -v MeisterAI 2>/dev/null || echo "$script_path")
            ;;
        meister|meister.sh)
    ...
    case "$(basename "$script_path")" in
        meisterSiri|MeisterSiri)
            script_path=$(command -v MeisterAI 2>/dev/null || echo "/opt/homebrew/bin/MeisterAI")
            ;;
    esac
...
PLISTEOF
    ...
    if [ "$ok" -ge 1 ]; then
        ...
        echo "  Daily:  09:15  ${daily_args}   (MeisterAI auto)"
...
if [ "${1:-}" = "sniff" ]; then
    ...
    while true; do
        ...
        printf '\033[1;34m'
        printf '  ╔══════════════════════════════════════════════════╗\n'
        printf '  ║  MeisterAI SNIFF — Live Network Monitor           ║\n'
        printf '  ╚══════════════════════════════════════════════════╝\n'
        printf '\033[0m\n'
...
if [ "${1:-}" = "ntop" ]; then
    ...
    while true; do
        ...
        printf '\033[1;34m'
        printf '  ╔══════════════════════════════════════════════════╗\n'
        printf '  ║  MeisterAI NTOP — Network Traffic Top 10          ║\n'
        printf '  ╚══════════════════════════════════════════════════╝\n'
        printf '\033[0m\n'
...
if [ "${1:-}" = "disk" ]; then
    TARGET="${2:-$HOME}"
    echo -e "\033[1;34m  MeisterAI DISK — Top Space Usage: $TARGET\033[0m"
    echo ""
    printf '  %10s  %s\n' "SIZE" "DIRECTORY"
    printf '  %10s  %s\n' "----" "---------"
...
if [ "${1:-}" = "ports" ]; then
    echo -e "\033[1;34m  MeisterAI PORTS — Open Ports & Listeners\033[0m"
    echo ""
...
if [ "${1:-}" = "dns" ]; then
    echo -e "\033[1;34m  MeisterAI DNS — DNS Leak Test\033[0m"
    echo ""
    # Current DNS servers
...
if [ "${1:-}" = "battery" ]; then
    echo -e "\033[1;34m  MeisterAI BATTERY — Battery Health\033[0m"
    echo ""
...
if [ "${1:-}" = "startup" ]; then
    echo -e "\033[1;34m  MeisterAI STARTUP — Login Items & Launch Agents\033[0m"
    echo ""
...
if [ "${1:-}" = "wifi" ]; then
    echo -e "\033[1;34m  MeisterAI WIFI — Wi-Fi Diagnostics\033[0m"
    echo ""
...
if [ "${1:-}" = "top" ]; then
    ...
    while true; do
        ...
        printf '\033[1;34m'
        printf '  ╔══════════════════════════════════════════════════╗\n'
        printf '  ║  MeisterAI TOP — Process Monitor                  ║\n'
        printf '  ╚══════════════════════════════════════════════════╝\n'
        printf '\033[0m\n'
...
if [ "${1:-}" = "certs" ]; then
    echo -e "\033[1;34m  MeisterAI CERTS — Certificate Checker\033[0m"
    echo ""
...
if [ "${1:-}" = "thermal" ]; then
    ...
    while true; do
        ...
        printf '\033[1;34m'
        printf '  ╔══════════════════════════════════════════════════╗\n'
        printf '  ║  MeisterAI THERMAL — Temperature & Fan Monitor    ║\n'
        printf '  ╚══════════════════════════════════════════════════╝\n'
        printf '\033[0m\n'
...
if [ "${1:-}" = "remove" ] || [ "${1:-}" = "uninstall" ]; then
    ...
    for _a in "$@"; do
        case "$_a" in
            ...
            -*)           echo "[ERROR] Unknown flag: $_a"; echo "Usage: MeisterAI remove <AppName> [--dry-run] [--purge] [-y]"; exit 1 ;;
            *)            [ -z "$REMOVE_NAME" ] && REMOVE_NAME="$_a" || REMOVE_NAME="$REMOVE_NAME $_a" ;;
        esac
    done
    if [ -z "$REMOVE_NAME" ]; then
        echo "Usage: MeisterAI remove <AppName> [--dry-run] [--purge] [-y]"
    ...
    echo -e "\033[1;34m  MeisterAI REMOVE — Uninstall app + leftovers\033[0m"
    echo ""
    DRY_RUN=$REMOVE_DRY
...
if [ "${1:-}" = "orphans" ]; then
    ...
    for _a in "$@"; do
        case "$_a" in
            ...
            -y|--yes)     ORPH_YES=true ;;
            -*) echo "[ERROR] Unknown flag: $_a"; echo "Usage: MeisterAI orphans [--dry-run] [--purge] [-y]"; exit 1 ;;
            *)  echo "[ERROR] Unexpected argument: $_a"; echo "Usage: MeisterAI orphans [--dry-run] [--purge] [-y]"; exit 1 ;;
        esac
    done

    echo -e "\033[1;34m  MeisterAI ORPHANS — Leftovers of uninstalled apps\033[0m"
    echo ""
    DRY_RUN=$ORPH_DRY
    $DRY_RUN && echo "  [DRY-RUN — no changes]" && echo ""
    shopt -s nullglob
...
if [ "${1:-}" = "simfix" ]; then
    echo -e "\033[1;34m  MeisterAI SIMFIX — Repair iOS Simulator\033[0m"
    echo ""
    DRY_RUN=false
...
if [ "${1:-}" = "tcc-clean" ]; then
    echo -e "\033[1;34m  MeisterAI TCC-CLEAN — verwaiste Privacy-Eintraege\033[0m"
    echo ""
...
if [ "${1:-}" = "appupdates" ] || [ "${1:-}" = "macupdate" ]; then
    echo -e "\033[1;34m  MeisterAI APPUPDATES — alle App-Updates (MacUpdater-Style)\033[0m"
    echo ""
    _AU_TOTAL=0
...
if [ "${1:-}" = "diff" ]; then
    echo -e "\033[1;34m  MeisterAI DIFF — Systemaenderungen seit letztem Snapshot\033[0m"
    echo ""
    SNAPSHOT_DIR="$MEISTER_DIR/snapshots"
    if [ "${2:-}" = "--snapshot" ]; then
...
if [ "${1:-}" = "score" ]; then
    echo -e "\033[1;34m  MeisterAI SCORE — Wartungs-Score-Verlauf\033[0m"
    echo ""
...
if [ "${1:-}" = "undo" ]; then
    echo -e "\033[1;34m  MeisterAI UNDO — letzte reversible Aktionen zuruecknehmen\033[0m"
    echo ""
...
if [ "${1:-}" = "explain" ]; then
    ...
    if [ -z "$_EX_TEXT" ]; then
        ...
        if [ -z "$_EX_TEXT" ]; then
            echo "Usage: MeisterAI explain <Warnung oder Log-Zeile>"; exit 1
        fi
    fi
    echo -e "\033[1;34m  MeisterAI EXPLAIN\033[0m"
    echo ""
    echo "  Meldung: $_EX_TEXT"
    echo ""
    if ! fm_available; then
        if [ "${AI_BACKEND_KIND:-apple}" = ollama ]; then
            echo "  Ollama nicht verfügbar — Server und MEISTER_OLLAMA_MODEL prüfen."
        else
...
In 3 kurzen Absaetzen: (1) Was bedeutet das? (2) Ist es gefaehrlich/dringend? (3) Konkrete Handlung, mit Befehl falls sinnvoll. Keine Einleitung."
    ...
    _AI_TEXT_RC=${PIPESTATUS[0]}
    [ "$_AI_TEXT_RC" -eq 0 ] || exit "$_AI_TEXT_RC"
    echo ""
    exit 0
fi
...
if [ "${1:-}" = "fleet" ]; then
    echo -e "\033[1;34m  MeisterAI FLEET — Status aller Macs\033[0m"
    echo ""
...
if [ "${1:-}" = "today" ] || [ "${1:-}" = "brief" ]; then
    echo -e "\033[1;34m  MeisterAI TODAY — $(date '+%A, %d. %B %Y %H:%M')\033[0m"
    echo ""
    # Score
    ...
    if fm_available; then
        ...
        _tp="Du bist MeisterAI. In EINEM kurzen deutschen Satz: Was sollte der User heute als erstes am Mac tun? Fakten: Score ${_sc:-?}/100, Disk ${_disk}, Brew outdated f=${_bo:-?} c=${_bc:-?}. Keine Einleitung."
        fm_query "$_tp" "today" 2>/dev/null | sed -n '1,3s/^/  → /p'
        _AI_TEXT_RC=${PIPESTATUS[0]}
        [ "$_AI_TEXT_RC" -eq 0 ] || echo "  AI focus nicht verfügbar (Exit $_AI_TEXT_RC); Tagesübersicht bleibt gültig."
    fi
    echo ""
    echo "  Next: MeisterAI doctor | MeisterAI -n | MeisterAI ai"
    exit 0
fi
...
if [ "${1:-}" = "doctor" ]; then
    ...
    echo -e "\033[1;34m  MeisterAI DOCTOR — Read-only checklist\033[0m"
    echo ""
    ...
    if touchid_sudo_enabled 2>/dev/null || { [ -f /etc/pam.d/sudo_local ] && grep -qE '^[[:space:]]*auth[[:space:]].*pam_tid' /etc/pam.d/sudo_local 2>/dev/null; }; then
        ...
        _check warn "Touch ID sudo" "off — auto-enable on next sudo (or: MeisterAI touchid)"
    else
        _check warn "Touch ID sudo" "off (TOUCHID_SUDO=false) — MeisterAI touchid"
    fi
    if [ -f /etc/sudoers.d/zz-meister ]; then
        _check ok "Sudo share" "zz-meister (2h, !tty_tickets)"
    else
        _check warn "Sudo share" "default tty tickets — MeisterAI sudo-setup"
    fi
...
if [ "${1:-}" = "privacy" ]; then
    echo -e "\033[1;34m  MeisterAI PRIVACY — Privacy grants quick audit\033[0m"
    echo ""
    ...
    if [ -r "$_tcc" ]; then
        ...
        echo "  then re-run. Deep clean: MeisterAI tcc-clean"
    fi
    echo ""
    ...
    echo ""
    echo "  Tip: MeisterAI tcc-clean --do  (orphaned privacy grants)"
    echo "       MeisterAI startup         (login items audit)"
    exit 0
fi
...
if [ "${1:-}" = "suggest" ]; then
    ...
    _q="$*"
    if [ -z "$_q" ]; then
        echo "Usage: MeisterAI suggest <problem description>"
        echo "  Read-only: ${AI_BACKEND_LABEL:-AI} proposes a fix, nothing is executed."
        exit 1
    fi
    echo -e "\033[1;34m  MeisterAI SUGGEST — AI fix idea (read-only)\033[0m"
    echo ""
    if ! fm_available; then
        echo "  ${AI_BACKEND_LABEL:-AI} nicht verfügbar."
        exit 1
    fi
    echo "  Problem: $_q"
    echo "  Frage ${AI_BACKEND_LABEL}..."
    echo ""
    _sp="Du bist MeisterAI, macOS-Wartungsassistent. Problem des Users: ${_q}
...
if [ "${1:-}" = "selftest" ]; then
    echo -e "\033[1;34m  MeisterAI SELFTEST — smoke tests\033[0m"
    echo ""
    _p=0; _f=0
    _t() {
        local name="$1"; shift
    ...
    _SELF="$0"
    _BIN=$(basename "$_SELF")
    # sync-twins rewrites "MeisterAI"→"meister"; build OTHER via concat so it survives.
    ...
    if fm_available; then
        printf "  \033[0;32mPASS\033[0m  %s online\n" "${AI_BACKEND_LABEL:-AI}"; _p=$((_p+1))
    else
        printf "  \033[1;33mWARN\033[0m  %s offline (env)\n" "${AI_BACKEND_LABEL:-AI}"
    fi
...
# Called by `MeisterAI autofix` and by `MeisterAI ai` before diagnosis.
autofix_known_issues() {
    local fix_count=0 skip_count=0
    echo -e "\033[1;34m  MeisterAI AUTOFIX — deterministische Fixes\033[0m"
    echo ""
        ...
        if [ "$fw" = disabled ]; then
            ...
            if $DRY_RUN; then
                ...
                if ensure_sudo "enable firewall" 2>/dev/null || sudo_has_ticket 2>/dev/null; then
                    ...
                    log WARN "   Firewall: need sudo (MeisterAI sudo-setup / Touch ID once)"
                    report_add WARN "   Firewall: need sudo (MeisterAI sudo-setup / Touch ID once)"
                    skip_count=$((skip_count + 1))
                fi
            fi
        elif [ "$fw" = enabled ]; then
            log STEP "   Firewall: already on"
        else
            log WARN "   Firewall: state unavailable"
            report_add WARN "macOS Firewall state could not be checked"
            skip_count=$((skip_count + 1))
...
if [ "${1:-}" = "ai-log" ] || [ "${1:-}" = "ai-usage" ] || { [ "${1:-}" = "ai" ] && { [ "${2:-}" = "usage" ] || [ "${2:-}" = "log" ]; }; }; then
    echo -e "\033[1;34m  MeisterAI AI USAGE — was kam vom Modell?\033[0m"
    echo ""
    ...
    echo "  purpose=explain        MeisterAI explain …     → nur Text erklären"
    echo "  purpose=today          MeisterAI today         → 1-Satz Fokus"
    echo "  purpose=suggest        MeisterAI suggest …     → Fix-Idee, nie ausführen"
    echo "  purpose=ai-diagnose    MeisterAI ai            → Rest-Risiken nach Autofix"
    echo "  purpose=ai-heal        Modulfehler → belegte Diagnose + geprüfte Aktions-ID"
    echo "  MeisterAI ai report  → gespeicherte Ergebnisse mit Belegverweisen"
    ...
    if [ ! -f "$_logf" ]; then
        echo "  (noch keine AI-Calls protokolliert — z.B. MeisterAI explain \"test\")"
        exit 0
    fi
    _n=$(wc -l < "$_logf" | tr -d ' ')
...
if [ "${1:-}" = ai ] && [ "${2:-}" = report ]; then
    fm_report_summary || exit 1
    exit 0
fi

if [ "${1:-}" = "ai" ]; then
    # MeisterAI ai suggest <text> → suggest only
    if [ "${2:-}" = "suggest" ]; then
        shift 2
        exec "$0" suggest "$*"
    fi
    ...
    set -- ai "${_AI_ARGS[@]}"
    # MeisterAI ai --diagnose-only | ai diagnose → no autofix
    _AI_DIAG_ONLY=false
    _AI_FOCUS=""
    ...
    if $_AI_DIAG_ONLY; then
        echo -e "\033[1;34m  MeisterAI AI — Nur Diagnose\033[0m"
    else
        echo -e "\033[1;34m  MeisterAI AI — Diagnose + Auto-Fix\033[0m"
    fi
    echo ""
    ...
    if $DRY_RUN; then
        echo "  Vorschau beendet; keine Modellabfrage oder Reparatur ausgeführt."
        RUN_STATUS=completed
        exit 0
    fi

    if ! fm_available "${FM_DIAGNOSE_MODEL:-system}"; then
        echo "  ${AI_BACKEND_LABEL:-AI} nicht verfügbar — keine Modell-Diagnose erstellt."
        echo "  Manuell: MeisterAI autofix | MeisterAI doctor"
        RUN_STATUS=completed
        exit 69
    fi
    # Saved report is labeled historical; current autofix outcomes are separate.
    _AI_FACTS=$(fm_report_summary 2>/dev/null || printf 'Kein gespeicherter Bericht verfügbar.')
    if ! $_AI_DIAG_ONLY; then
        _AI_FACTS="$_AI_FACTS
Aktueller Autofix — gemeldete Maßnahmen: ${REPORT_FIXED[*]}
Aktuell offen: ${REPORT_WARNINGS[*]} ${REPORT_ERRORS[*]}"
    fi
    _AI_RC=0
    fm_diagnose "${_AI_FOCUS:-System diagnosis}" "$_AI_FACTS" "" ai-diagnose || _AI_RC=$?
    if [ "$_AI_RC" -ne 0 ]; then
        printf '%s\n' "$_AI_FACTS"
        RUN_STATUS=completed
        exit "$_AI_RC"
    fi
    echo "  Diagnose ist ein Vorschlag; Maßnahmen und Verifikation stehen im gespeicherten Bericht."
...
if [ "${1:-}" = "pkg" ]; then
    ...
    if [ -z "$_PKG" ] || [ ! -f "$_PKG" ]; then
        echo "Usage: MeisterAI pkg <file.pkg>"; exit 1
    fi
    echo -e "\033[1;34m  MeisterAI PKG — Installer-Inspektor\033[0m"
    echo ""
...
WATCHEOF
        ...
        --accept)
            ...
            ;;
        *)
            echo -e "\033[1;34m  MeisterAI WATCH — Persistence-Waechter (BlockBlock-Style)\033[0m"
            echo ""
...
if [ "${1:-}" = "tweaks" ]; then
    ...
    _T="${2:-}"; _V="${3:-on}"
    if [ -z "$_T" ]; then
        echo -e "\033[1;34m  MeisterAI TWEAKS — versteckte macOS-Einstellungen (OnyX-Style)\033[0m"
        echo ""
        _tweak_status
        echo ""
...
if [ "${1:-}" = "adopt" ]; then
    echo -e "\033[1;34m  MeisterAI ADOPT — Apps unter Homebrew-Verwaltung bringen\033[0m"
    echo ""
...
if [ "${1:-}" = "dash" ]; then
    ...
    while true; do
        ...
        _D_PREV=$_D_CUR
        clear
        echo -e "\033[1;34m  MeisterAI DASH — $(date '+%H:%M:%S')   (q beendet, Intervall ${_D_INT}s)\033[0m"
        echo ""
...
if [ "${1:-}" = "files" ]; then
    _F="${2:-}"
    if [ -z "$_F" ]; then
        echo "Usage: MeisterAI files <port|prozessname|pfad>"
    ...
    echo -e "\033[1;34m  MeisterAI FILES — offene Dateien/Ports (Sloth-Style)\033[0m"
    echo ""
...
if [ "${1:-}" = "win" ]; then
    _W_POS="${2:-}"
    if [ -z "$_W_POS" ]; then
        echo "Usage: MeisterAI win <left|right|max|center|tl|tr|bl|br>"
...
CLIPEOF
        ...
        ''|*[0-9]*)
            ...
            echo -e "\033[1;34m  MeisterAI CLIP — Clipboard-History (Maccy-Style)\033[0m"
            echo ""
            awk -v RS="$(printf '\x1e')----MEISTERCLIP----\n" \
...
KEYSEOF
    ...
    case "$_K_MODE" in
        ...
        status)
            echo -e "\033[1;34m  MeisterAI KEYS — Tastatur-Remapping (hidutil)\033[0m"
            echo ""
        ...
        *) echo "Usage: MeisterAI keys [caps2esc|caps2ctrl|reset|status]"; exit 1 ;;
    esac
    exit 0
fi
...
if [ "${1:-}" = "touchid" ]; then
    echo -e "\033[1;34m  MeisterAI TOUCHID — Touch ID for sudo\033[0m"
    echo ""
    ...
    if touchid_sudo_enabled 2>/dev/null || grep -qE '^[[:space:]]*auth[[:space:]].*pam_tid' "$_PAM_LOCAL" 2>/dev/null; then
        ...
        echo "  Undo: MeisterAI touchid --off"
        exit 0
    fi
    ...
    if touchid_sudo_enabled 2>/dev/null || grep -qE '^[[:space:]]*auth[[:space:]].*pam_tid' "$_PAM_LOCAL" 2>/dev/null; then
        ...
        echo "  Default: every MeisterAI/meister run keeps this on (TOUCHID_SUDO=true)"
...
if [ "${1:-}" = "homebrew-share" ]; then
    ...
    echo -e "\033[1;34m  MeisterAI HOMEBREW-SHARE — prefix for every admin user\033[0m"
    echo ""
...
# ── twins-bench — meister vs MeisterAI (Ollama vs Apple Intelligence) ──
...
if [ "${1:-}" = "why" ]; then
    echo -e "\033[1;34m  MeisterAI WHY\033[0m"
    echo ""
...
if [ "${1:-}" = "storage" ]; then
    echo -e "\033[1;34m  MeisterAI STORAGE — safe-to-delete candidates\033[0m"
    echo ""
...
if [ "${1:-}" = "contacts" ]; then
    echo -e "\033[1;34m  MeisterAI CONTACTS\033[0m"
    echo ""
    case "${2:-doctor}" in
        doctor|health|status|"")
            ...
            ;;
        *)
            echo "Usage: MeisterAI contacts doctor"
            exit 1
            ;;
    esac
    exit 0
...
if [ "${1:-}" = "report" ]; then
    ...
    if [ "$_N" = "--diff" ] || [ "${2:-}" = "--diff" ]; then
        echo -e "\033[1;34m  MeisterAI REPORT --diff\033[0m"
        echo ""
    ...
    case "$_N" in *[!0-9]*) echo "Usage: MeisterAI report [N|--diff|--json [N]]"; exit 1 ;; esac
    _hist="$MEISTER_DIR/history.log"
    [ -f "$_hist" ] || { echo "  No history yet ($_hist)"; exit 0; }
    echo -e "\033[1;34m  MeisterAI REPORT — last ${_N} runs\033[0m"
    echo ""
...
if [ "${1:-}" = "free" ]; then
    echo -e "\033[1;34m  MeisterAI FREE — Free up RAM & reset UI\033[0m"
    echo ""
...
# ── Healer (meister heal) ──
if [ "${1:-}" = "heal" ]; then
    echo -e "\033[1;34m  MeisterAI HEAL — Auto-Healing\033[0m"
    echo ""
    DRY_RUN=false
    [ "${2:-}" = "--dry-run" ] && DRY_RUN=true
...
if [ "${1:-}" = "speed" ]; then
    echo -e "\033[1;34m  MeisterAI SPEED — Network Speed Test\033[0m"
    echo ""
        ...
        --*)
            case "$_FIRST_POS" in
                bloatware|kill-bloat|bloat|remove|uninstall|orphans|report|doctor|watch|clip|keys|ai|heal|free)
                    _NEW_ARGS+=("$arg") ;;
                *)
                    echo "[ERROR] Unknown option: $arg (see MeisterAI -h)"; exit 1 ;;
            esac
            ;;
        *)         _NEW_ARGS+=("$arg") ;;
    esac
...
while getopts ":aAXTSCLhcHnIPGNq" opt; do
  case $opt in
    ...
    h) cat << 'HELPEOF'
MeisterAI - macOS Maintenance, Self-Healing & Dotfiles Sync (Apple Intelligence)

MAINTENANCE:
  MeisterAI              Auto-detect daily-fast (default profile=auto)
  MeisterAI --quick      Lean daily (~1-2 min): healer, brew, cleanup, security
  MeisterAI --deep       Weekly full: iCloud, dev caches, docs, all audits
  MeisterAI --auto       Explicit daily-fast defaults
  MeisterAI menu         Interactive menu (TUI)
  MeisterAI -a           Force ALL modules (profile=all)
  MeisterAI -n           Dry-run
...
TOOLS:
  MeisterAI sniff [N]    Live network monitor (default: 3s refresh)
  MeisterAI ntop [N]     Live network traffic top 10 (default: 3s)
  MeisterAI disk [dir]   Disk space analyzer (default: ~)
  MeisterAI ports        Open ports & listeners
  MeisterAI dns          DNS leak test
  MeisterAI battery      Battery health report
  MeisterAI heal [--dry-run]  Proactive auto-healer (broken symlinks, orphans, DNS, casks)
  MeisterAI free [--restart-ui]  Free RAM (sudo purge) + optionally restart Finder/Dock
  MeisterAI simfix       Stop stale Simulator.app (no simctl catalog, does not boot)
  MeisterAI startup      Login items & launch agents audit
  MeisterAI wifi         Wi-Fi diagnostics & channel scan
  MeisterAI top [N]      Live process monitor (default: 3s refresh)
  MeisterAI certs [host] SSL certificate checker
  MeisterAI thermal [N]  Live temperature & fan monitor (default: 2s)
  MeisterAI speed        Download/upload speed test
  MeisterAI report [N]   Run-history report from history.log (default: last 10)
  MeisterAI report --diff  Delta between last two runs (OK/FIX/WARN/ERR/SCORE)
  MeisterAI report --json [N]  History as JSON array
  MeisterAI why <x>      Why a module runs/skips, or explain a warn (profile|Homebrew|…)
  MeisterAI storage      Safe-to-delete size candidates (DerivedData, caches, …)
  MeisterAI contacts doctor  AddressBook size / bloat risk (read-only)
  MeisterAI score        Maintenance score 0-100 + trend history
  MeisterAI diff         What changed since the last run (apps/autostart/brew)
  MeisterAI undo [--do]  Revert the last run's reversible actions (--list)
  MeisterAI explain <x>  Apple Intelligence explains a warning in plain language
  MeisterAI today        Morning briefing (score, AI, disk, brew, top warns)
  MeisterAI doctor       Read-only system checklist (security, brew, TM, AI)
  MeisterAI doctor --json  Same signals as machine-readable JSON
  MeisterAI twins-bench [--json] [--quick]  Benchmark meister vs MeisterAI
  MeisterAI suggest <x>  AI fix suggestion only — never executes
  MeisterAI privacy      Privacy grants / persistence quick audit
  MeisterAI selftest     Smoke-test this CLI
  MeisterAI fleet        Score/status of all Macs (FLEET_HOSTS in config)
  MeisterAI touchid [--off|status]  Touch ID for sudo (auto-on; pam_tid in sudo_local)
  MeisterAI sudo-setup       Long shared sudo ticket (2h, all terminals)
  MeisterAI homebrew-share   Share the Homebrew prefix with every admin user
  MeisterAI backup [--now]   Time Machine status; set up destination if none
  MeisterAI dash [N]     Live system dashboard: CPU/RAM/Disk/Netz (Stats-style)
  MeisterAI files <x>    Who has port/file/process open (Sloth-style lsof)
  MeisterAI ai           Autofix known issues + AI summary of remaining risks
  MeisterAI ai usage     Was hat Apple Intelligence / Ollama wann gemacht?
  MeisterAI ai-log       Alias für ai usage
  MeisterAI ai --diagnose-only   AI only (no autofix)
  MeisterAI autofix      Deterministic fixes (bottles, orphans, git push, FW, inbox)

SECURITY:
  MeisterAI pkg <file>   Inspect .pkg BEFORE install: signature, payload, scripts
  MeisterAI watch        Persistence watcher: notify on new LaunchAgents/Daemons
                       (--install / --uninstall / --check)
  MeisterAI tcc-clean [--do]  Remove privacy grants of DELETED apps (FDA list etc.)

SYSTEM:
  MeisterAI tweaks       Hidden macOS settings (OnyX-style): showhidden,
                       extensions, pathbar, keyrepeat, savepanel, dockfast
  MeisterAI adopt [--do] Bring unmanaged /Applications apps under brew (updates!)
  MeisterAI appupdates   ALL app updates in one list: brew + App Store + Sparkle
  MeisterAI win <pos>    Move frontmost window: left|right|max|center|tl|tr|bl|br
  MeisterAI clip         Clipboard history (Maccy-style): --install, <nr>, --purge
  MeisterAI keys <mode>  Key remapping: caps2esc | caps2ctrl | reset | status

APP MANAGEMENT:
  MeisterAI remove <App> [--dry-run] [--purge] [-y]
                       Uninstall app + all leftovers (caches, prefs, containers,
                       saved state, logs). Default: to Trash (reversible). --purge: rm.
  MeisterAI orphans [--dry-run] [--purge] [-y]
                       Scan ~/Library + /Library for leftovers of apps that are no
                       longer installed; pick which to remove. Default: Trash.
  MeisterAI bloatware [scan|kill] [--p0|--p1] [--dry-run] [-y] [--json|--tsv]
...
DOTFILES SYNC:
  MeisterAI push         Collect configs, commit, push
  MeisterAI pull         Pull latest, create symlinks
  MeisterAI setup [url]  Clone dotfiles repo (auto-detects from gh)
  MeisterAI init [name]  Create private GitHub repo + push
  MeisterAI scan         Auto-detect configs, generate manifest
  MeisterAI clone        Clone ~/Developer repos
  MeisterAI bootstrap    Full setup: pull + brew + npm + clone + defaults
...
Docs: docs/PRODUCT.md  docs/GUI.md
  ...
  Tip: MeisterAI --quick daily · MeisterAI --deep weekly
HELPEOF
       exit 0 ;;
...
if ! $DRY_RUN && $NEEDS_SUDO; then
    if ensure_sudo "full maintenance run"; then
        log INFO "   Sudo: ready (shared ticket — meister + MeisterAI reuse it)"
    else
...
if ! $DRY_RUN && fm_available "${FM_HEAL_MODEL:-auto}"; then
...
if $DRY_RUN || check_net; then
    ...
    if $RUN_SUDO_TASKS; then
        ...
        _sm_rc=0
        module_system_maintenance || _sm_rc=$?
        module_timer_stop "System maintenance"
        ledger_add "System maintenance" "$_sm_fix0" "$_sm_warn0" "$_sm_err0" "$_sm_rc"
        fi
    fi
    RUN_STATUS=completed
else
