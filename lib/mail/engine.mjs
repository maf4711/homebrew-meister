...
export class MegasmartEngine {
  constructor(mail, stateDir, { previewLimit = Infinity, previewPageSize = 100, rotatePreview = false, verificationLimit = Infinity, persist = durableJSON, onProgress = () => {}, classifier, localHeaders, previewReader } = {}) {
    if (!Number.isInteger(previewPageSize) || previewPageSize < 1 || previewPageSize > 100) throw new Error('Preview page size must be between 1 and 100');
    this.previewPageSize = previewPageSize;
    this.rotatePreview = rotatePreview;
  ...
  async preview(account, mailbox, afterJobID, trashMailbox) {
    ...
    // Continue from the last attempted model row, scoped to this inbox and
    // classifier policy. Cached results later in the scan must not skip deferred work.
    const previous = this.rotatePreview ? [...this.jobs.values()].reverse().filter(j =>
      j.account === account && j.mailbox === mailbox && j.policy?.classifierVersion === policy?.classifierVersion &&
      Number.isSafeInteger(j.nextPreviewOffset) && j.nextPreviewOffset >= 0).sort((a, b) => b.createdAt.localeCompare(a.createdAt))[0] : null;
    const anchor = previous?.nextPreviewMessageID ? listing.messages.findIndex(row => row.id === previous.nextPreviewMessageID) : -1;
    const start = anchor >= 0 ? anchor : listing.messages.length ? (previous?.nextPreviewOffset ?? 0) % listing.messages.length : 0;
    const ordered = [...listing.messages.slice(start), ...listing.messages.slice(0, start)];
    const positions = new Map(listing.messages.map((row, index) => [row.id, index]));
    let nextPreviewOffset = start;
    const items = [];
    const startedAt = performance.now();
    let readMilliseconds = 0, modelMilliseconds = 0;
    const report = activity => this.onProgress({ phase: 'preview', activity, checked: items.length, total: listing.messages.length,
      ...
      modelStats: this.classifier?.stats });
    report('Mail lesen');
    const pages = [];
    for (let base = 0; base < ordered.length; base += this.previewPageSize) pages.push(ordered.slice(base, base + this.previewPageSize));
    ...
    for await (const { page, eligible, details } of prepared) {
      ...
      report(this.classifier && modelRows.length ? `FM prüft ${modelRows.length}` : 'Mail lesen');
      ...
      if (this.classifier && (classified.length !== modelRows.length || new Set(classified.map(r => r.id)).size !== modelRows.length || classified.some(r => !modelRows.some(row => row.id === r.id)))) throw new Error('Incomplete Apple FM classification');
      for (const result of classified) {
        if (result.attempted && !result.deferred) nextPreviewOffset = (positions.get(result.id) + 1) % listing.messages.length;
      }
      ...
      if (localMoves.length) {
        report('Mail bestätigt Kandidaten');
        const confirmationStarted = performance.now();
        const nativeDetails = new Map();
        for (let base = 0; base < localMoves.length; base += 25) {
          const chunk = localMoves.slice(base, base + 25);
          try {
            for (const [id, detail] of await this.batchDetails(account, mailbox, chunk)) nativeDetails.set(id, detail);
          } catch (error) {
            if (error.code !== 'MAIL_READ_TIMEOUT') throw error;
            // Only this unconfirmed native batch is retained. Continue other
            // batches without ever creating a move approval for timed-out rows.
            for (const row of chunk) {
              details.set(row.id, { ...details.get(row.id), nativeUnverified: true });
              classifications.delete(row.id);
            }
          }
        }
        ...
        for (const row of localMoves) {
          ...
          if (local.nativeUnverified) continue;
        ...
        if (changed.length) {
          report(`FM bestätigt ${changed.length}`);
      ...
      if (!decision.protected && decision.action !== 'keep' || (!decision.protected && !decision.destination)) {
        ...
        if (detail.nativeUnverified) decision = { action: 'keep', destination: '', protected: true, reason: 'Native content confirmation timed out; message remains unverified' };
      ...
      if (classification?.deferred) decision = { action: 'keep', destination: '', protected: true, reason: 'Model budget reached; deferred until a later run', deferred: true };
      ...
      items.push({ id: randomUUID(), messageID: row.id, sender: row.sender, subject: row.subject, ...(policy ? { dateReceived: row.dateReceived } : {}),
        ...decision, ...(detail?.nativeUnverified ? { nativeUnverified: true } : {}), ...(detail?.localUnavailable ? { localUnavailable: true } : {}), ...(classification ? { classification } : {}), status: 'pending', ...(detail && !detail.localPreview ? { fingerprint: identity(row, detail), rfcMessageId: detail.rfcMessageId } : {}) });
    }
      report(items.length === listing.messages.length ? 'Durchsicht abgeschlossen' : 'Mail lesen');
    }
    const job = { id: randomUUID(), account, mailbox, ...(this.rotatePreview ? { nextPreviewOffset, nextPreviewMessageID: listing.messages[nextPreviewOffset]?.id } : {}), createdAt: new Date().toISOString(), status: 'preview', items, scanned: items.length, hasMore: listing.hasMore, offset, ...(listing.source ? { source: listing.source, headersScanned: listing.headersScanned } : {}), ...(policy ? { policy } : {}) };
