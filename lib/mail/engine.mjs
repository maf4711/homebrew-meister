  ...
  async preview(account, mailbox, afterJobID, trashMailbox) {
    ...
    for await (const { page, eligible, details } of prepared) {
      ...
      if (localMoves.length) {
        ...
        let nativeDetails;
        try { nativeDetails = await this.batchDetails(account, mailbox, localMoves); }
        catch (error) {
          if (error.code !== 'MAIL_READ_TIMEOUT') throw error;
          // Only a read timeout during preview can skip candidates. Keep the
          // whole unconfirmed batch; never create fingerprints or move approvals.
          nativeDetails = new Map();
