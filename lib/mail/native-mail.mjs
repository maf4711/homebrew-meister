...
async function defaultRunner(payload) {
  const { stdout } = await exec('/usr/bin/osascript', ['-l', 'AppleScript', helper, JSON.stringify(payload)], { timeout: 120000, maxBuffer: 32 * 1024 * 1024 });
...
export class NativeMail {
  ...
  async perform(payload) {
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      throw new Error(result?.error || 'Incomplete native Mail operation');
    }
  }
